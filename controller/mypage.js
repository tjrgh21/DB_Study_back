...
exports.postAddress = async (req, res) => {
    const { address_id, postal_code, address, detailed_address } = req.body.address
    console.log(`주소id: ${address_id}, 우편번호: ${postal_code}, 기본주소: ${address}, 상세주소: ${detailed_address}`)

    const user = req.session.user

    const addAddress = await pool.query("INSERT INTO address VALUES(?,?,?,?,?)", [address_id, postal_code, address, detailed_address, user])
    res.send({ msg: "주소 추가" })
}
