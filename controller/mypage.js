...
exports.deleteCard = async (req, res) => {
    ...
    const user = req.session.user
    await pool.query("DELETE FROM credit WHERE card_id = ? AND user_id = ?", [card_id.card_id, user]);
    res.send({ msg: "카드 목록에서 삭제" });
    console.log("카드 삭제:", card_id.card_id, "번 째 카드")
    // console.log("유저 정보:", user)
}
...
exports.deleteAddress = async (req, res) => {
    ...
    await pool.query("DELETE FROM address WHERE address_id = ? AND user_id =?", [address_id.address_id, user]);
